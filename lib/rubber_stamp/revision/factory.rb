class Revision::Factory < Revision
  ...
  def build_attribute(attr)
    return build_diff_attributes(attr) if attr_is_diff?(attr)
  ...
  def build_diff_attributes(attr)
    version_attribute = version_attributes.build(
      name: attr.key,
      old_value: previous_value(attr.key))
    Revision::DiffAttribute::Factory.new(
      version: version,
      attr: attr,
  ...
  def attr_is_diff?(attr)
    versionable.class.diff_attribute_names.include?(attr.key)
  end
